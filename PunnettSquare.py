...
genes = {
    #"Gene Name": {
    "Blood Type": {
        #"Allele Name": ["Symbol", #Dominance(0 being highest dominant), "(Optional) gene to append to (see Rh Factor gene. Leave empty otherwise.)"]
        "A": ["A", 0],
        "B": ["B", 0], #Example of codominance with A. (Using an equal Dominance #).
        "O": ["O", 1]
        },
    "Rh Factor": {
        "Rh+": ["^+", 0, "Blood Type"], #Example of allele that pairs with another gene. Uses the third "Optional gene to append to" option.
        "Rh-": ["^-", 1, "Blood Type"]
        },
    "Fruit Fly Eye Colour": {
        "E1": ["E_1", 0], #Example of many alleles.
        "E2": ["E_2", 1],
        "E3": ["E_3", 2],
        "E4": ["E_4", 3]
        } #Notice that there are no commas on the last items (in both genes (Fruit Fly Eye Colour) and the alleles (e.g. E4) themselves).
    }


genotype_1 = { #Here's our weird human-fly example (obviously no human would have fruit fly alleles as in this demo).
    #"Gene Name": ["Allele Name", "Allele Name"]
    "Blood Type": ["A", "A"],
    "Rh Factor": ["Rh+", "Rh+"],
    "Fruit Fly Eye Colour": ["E1", "E4"]
}

genotype_2 = { #It's important to use exact names, as "Blood Type" is not equal to "BloodType" or "Blood_Type"...Remember your allele and gene names.
    "Blood Type": ["B", "O"],
    "Rh Factor": ["Rh+", "Rh-"],
    "Fruit Fly Eye Colour": ["E2", "E3"]
}
